...
| itch.io on macOS | The `.app` goes up under a **new folder per release**, `v<version>/Boomtown.app` | The itch app patches a changed file in place, and macOS kills a signed binary that was rewritten in place (`Code Signature Invalid`), so every itch update crashed the game until a reinstall or reboot. New paths make every file a fresh write; butler still ships a patch because it diffs by content. See *macOS: a new folder for every release* in `deploying.md` |
