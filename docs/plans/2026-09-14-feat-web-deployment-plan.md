...
- **itch.io account identity, as a gate on online play.** The itch app can pass a game an `ITCHIO_API_KEY` when a manifest action requests `profile:me` scope, and the room could verify it against itch's API to learn which itch.io account is playing and whether it owns a copy. itch names this project's exact problem as the use case: restricting online play to legitimate owners. **Deferred, with a revisit trigger** (below) rather than rejected, because it is the one identity mechanism available that costs the player nothing and asks this project to store nothing.
  ...
  4. **Pay-what-you-want makes the check weak, and that is settled rather than open.** "Owns a copy" is only ever as strong as the purchase behind it. Boomtown is pay-what-you-want with no floor, so a player claims a copy for nothing and the check collapses into "has an itch.io account" — free, instant, and repeatable, which is near-zero resistance to someone minting identities. The apparent escape, verifying that a player actually *paid*, is worse than useless here: it would exclude most legitimate players, which is precisely what pay-what-you-want exists to allow. So itch's headline use case for this API — restricting online play to people who bought the game — is not a lever this project has, by choice rather than oversight.

     The knock-on runs the other way too. No purchase friction means no ceiling on how many people can reach a room, so the resource controls (R18, R19) carry more weight here than they would for a paid game, not less.
  ...
  **Revisit when:** hosts are fielding enough unwanted knocks that they want help judging them — observed, not anticipated. That is the whole trigger. The pricing half of it is gone: pay-what-you-want is a deliberate product choice, so "wait until it is paid" is not a condition that will arrive, and this should not sit in the plan pretending to wait for it.
