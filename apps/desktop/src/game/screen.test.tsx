...
describe('GameScreen turn gating', () => {
  ...
  it('holds the end screen until the victory beat has announced the winner', async () => {
    // The end screen prints the winner and the standings. `status` flips to
    // `over` in the same commit the `game-over` event lands, one commit before
    // the beat that reveals them can be queued — so gating on `status` alone
    // put the result on screen ahead of its own reveal (~1.2s of it, measured
    // in a real bot game).
    const { client } = await mount([0, 2], 1);

    await act(async () => {
      const result = {
        rankings: [
          { seat: 2, cash: 5000, equity: 3000, total: 8000, holdings: [] },
          { seat: 0, cash: 4000, equity: 1000, total: 5000, holdings: [] },
          { seat: 1, cash: 3000, equity: 500, total: 3500, holdings: [] },
        ],
        winners: [2],
      };
      const views = client.store.getState().views;
      const over = Object.fromEntries(
        Object.entries(views).map(([k, v]) => [k, { ...v, status: 'over' as const, result }]),
      );
      // the real shape: the event and the status arrive together (`endGame`)
      client.store.setState((state) => ({
        ...state,
        status: 'over' as const,
        views: over,
        log: [...state.log, { type: 'game-over' as const, result }],
      }));
    });
    await flush();
    ...
    // the board is what stands in for it — finished, and giving nothing away
    expect(screen.getByRole('grid', { name: 'Board' })).toBeInTheDocument();
