...
function BuyRow({
  ...
  return (
    <>
      ...
      <button type="button" aria-label={fill(copy.buy.oneFewer, { name })} disabled={qty === 0} onClick={onLess}>
        −
      </button>
      <output aria-label={fill(copy.buy.sharesToBuy, { name })}>{qty}</output>
      <button type="button" aria-label={fill(copy.buy.oneMore, { name })} disabled={!canAdd} onClick={onMore}>
        +
      </button>
    </>
  );
